...
**なぜ構造的に減らないのか**
- Claude Code は拡張思考（thinking）を有効にして動作し、Anthropic API は thinking 中のツール強制（`tool_choice`）を拒否する。さらにツール一覧や `tool_choice` を変えるとプロンプトキャッシュが無効になる。このためプロキシにできるのは、ツールを削らずに次の一手を助言することだけである。（2026-09-26 訂正：Anthropic の公式ドキュメントでは、拒否されるのは手動の extended thinking 中と、全モードの Opus 5.5 に限られ、他モデルの adaptive thinking では強制できる。`tool_choice` の変更で無効になるのはメッセージ部分のキャッシュ、ツール定義の変更ではキャッシュ全体。）
...
利用者の指示により、Codex のツール結果切り詰めを既定で有効（閾値20000バイト固定）にした。環境変数は on/off の切替 `JEV_CODEX_TOOL_OUTPUT_TRUNCATE`（既定 on）だけとし、数値の `JEV_CODEX_TOOL_OUTPUT_MAX` は廃止した。ベンチの比較条件は `--codex-tool-output-truncate` に置き換えた。上の６ペアの結果は閾値20000の同じ規則による。

### 区切り（2026-09-25）

利用者の判断により、Codex のツール結果切り詰めは一度完了とする。到達点は、既定で有効（閾値20000バイト、`JEV_CODEX_TOOL_OUTPUT_TRUNCATE=off` で無効）、負荷試験 `large-facts` の６ペアで総トークン削減率の中央値 +8.19%（区間はゼロをまたぎ `hold`）。未検証のまま残すのは、実作業全体での削減率と、再取得による待ち時間の悪化。README の `## Codex` 節に経緯を記載した。次の改善対象はコンパクション。

## 主指標から Jev を除外（2026-09-25）

利用者の指示により、トークン削減の主指標を「エージェント（親・子）の上流入力（キャッシュ込み）＋上流出力」に改め、Jev の入出力を除外した。Jev は利用料が非常に安く、計算に入れる必要がないため。本メモのこれまでの主 KPI（上流＋Jev 入出力）による判定は、Jev 込みの値である点に注意する。ベンチの `comparison.json`・効果判定・ダッシュボードも同じ定義に揃えた。Jev の使用量は別掲として残す。

## Codex：jev-gateway 方式のツール誘導の移植と事前登録（2026-09-25）

**実装**：`JEV_CODEX_STEER=on`（既定 off）で、Codex の Responses 要求のツール一覧を保ったまま、Jev の判定から `tool_choice` を `forced`（function／custom の型を区別）または `none` に書き換える。確信度0.7未満、「どのツールか」と「ツールが必要か」の判定の矛盾、namespace／hosted の選択、`agent_message` を含む会話、`previous_response_id` は転送する。上流が400／422を返したら原文を1回再送する。候補の絞り込みと費用ゲートは使わない。jev-gateway 0.3.1 の `decide.js` などに合わせた。`direct`・`hint`・120件超の2段階選定・圧縮本文の解凍は移植していない。根拠は、jev-gateway 本体を公表条件（Codex 0.154、`gpt-5.6-sol`、`chess-bugfix`、素の環境）で5ペア再現し、上流の要求数 −31%・入力 −30%・出力 −31%（中央値）を得たこと。

**パイロット**（効果判定に使わない）：`chess-bugfix` 1ペア、両側 36/36。介入の誘導は forced 29・none 1・転送0、上流の拒否0。上流入出力 1,448,879→1,390,928、要求 33→30。対照側がサブエージェントを1回呼び、子セッションを確認できず `child_session_unverified` で比較不能。

**本計測の条件（結果を見る前に固定）**：`gpt-5.6-terra`／`medium`、Codex 0.158、`--tasks chess-bugfix --modes off,on --codex-steer --reps 6`（順序交替）。主指標は上流入力（キャッシュ込み）＋上流出力の削減率 `100×(対照−介入)/対照` の中央値。Jev の使用量は別掲。比較可能条件は、両側品質満点、上流使用量の照合成功、欠測・汚染なし、同じ比較キー、`child_session_unverified` なし。比較可能ペアが6に満たない場合は、同じ条件でペアを追加し、比較可能ペアが6に達した時点で止める（総ペアの上限は10。上限でも6に届かなければ判定不能）。採用目安は、介入側の品質悪化なし・比較可能6ペア・削減率中央値5%以上。全ペアの差、範囲、要求数、キャッシュ内外の入力、経過時間、モード内訳、サブエージェント呼出しも報告する。

### 本計測の結果：判定不能（計測側の誤検出）

`/tmp/jev-codex-steer-reps6/` の6ペアは、全ペアが `child_session_unverified` で比較可能0だった（rep6 は使用量の欠測も重なった）。追加の4ペアで比較可能6に届く可能性がなくなったため、追加は1ランで止めた（`/tmp/jev-codex-steer-extra/`）。事前登録の規則どおり、判定不能とする。原因は計測側にある。`gpt-5.6-terra` はほぼ毎回 `spawn_agent` を呼ぶが、Codex 内部で失敗し（`collab spawn failed: no thread with id`）、子の要求は発生していない。それをプロキシが、結果がエラーでも起動成功と記録し、ベンチが子セッション未確認として比較不能にしていた。効果の数値は見ていない。起動失敗を数えないように修正した。

### 再登録：修正後の６ペア（結果を見る前に固定）

計測の修正以外は前の登録と同じ条件とする：`gpt-5.6-terra`／`medium`、Codex 0.158、`--tasks chess-bugfix --modes off,on --codex-steer --reps 6`（順序交替）。主指標は上流入力（キャッシュ込み）＋上流出力の削減率の中央値で、Jev は別掲。比較可能条件は、両側品質満点、上流使用量の照合成功、欠測・汚染なし、同じ比較キー、`child_session_unverified` なし（起動失敗は子セッションとして数えない）。比較可能ペアが6に満たなければ同じ条件でペアを追加し、比較可能6で止める（総ペアの上限10。届かなければ判定不能）。採用目安は、介入側の品質悪化なし・比較可能6ペア・削減率中央値5%以上。前の系列の6ペアは効果判定に使わない。

### 修正後の６ペア：結果と判断

保存先は `/tmp/jev-codex-steer-v2/`、追加分は `-extra/`〜`-extra3/`（総9ペア）。事前登録どおり、比較可能ペアが6に達した時点で停止した。品質は全18ランで36/36。比較不能の3ペア（v2.4、extra.1、extra2.1）は、使用量の欠測・上流使用量の照合失敗・子セッション未確認が重なった。extra.1 の介入側は `agent_message` による転送が26件あった。`spawn_agent` の起動失敗（`collab spawn failed:`）は18ラン中12回あり、修正後は1件もサブエージェント呼出しに数えていない。

| ペア | 上流総量 対照→介入 | 削減率 | 要求数 対照→介入 | 介入の forced／none／転送 |
|---|---|---:|---|---|
| v2.1 | 909,217→1,801,139 | −98.10% | 20→39 | 30／1／8（低確信度） |
...
**事前登録どおりの判定：削減率中央値 −37.08%（増加）、最小 −98.10%、最大 +10.35%、改善1・悪化5。採用しない。`JEV_CODEX_STEER` は既定 off のままとする。** 増加は主に上流要求数の増加による（最大で約2倍）。キャッシュ外入力はほぼ同じ水準だった。介入側は、ほぼ全要求で `exec` を強制した。jev-gateway 本体を同じ構成（terra、Codex 0.158）で動かした1ペア（要求 19→42）とも一致する。jev-gateway の公表条件（`gpt-5.6-sol`、Codex 0.154）での再現結果（要求 −31%）は、この構成では成り立たない。モデルと Codex 版のどちらが原因かは未分離である。

**原因の切り分け（診断、採否には使わない）**：terra の系列と同じ条件で、モデルだけを `gpt-5.6-sol` に替えて5ペア実行する（`/tmp/jev-codex-steer-sol/`）。Sol でも要求数が増えれば Codex 0.158（または本ベンチの構成）が原因、減れば terra 固有と判断する。追加・打切りはしない。

### 原因の切り分け：Sol の結果（利用者の指示で3ペアで打切り）

保存先は `/tmp/jev-codex-steer-sol/`。条件は terra の系列と同じで、モデルだけ `gpt-5.6-sol`／`medium` にした。利用者の指示により、欠測・照合失敗・子セッション未確認による比較不能の判定は無視し、上流（入力＋出力）の生のトークン量を比べる。品質は全6ランで合格した。

| ペア | 上流総量 対照→介入 | 削減率 | 要求数 対照→介入 |
|---|---|---:|---|
| 1 | 3,038,822→1,357,950 | +55.3% | 68→32 |
| 2 | 2,092,434→2,431,213 | −16.2% | 46→51 |
| 3 | 2,180,823→1,769,076 | +18.9% | 44→44 |

3ペアの削減率の中央値は +18.9%（改善2・悪化1）。4ペア目の途中で、利用者の指示により打ち切った。

## Codex のツール誘導：試行錯誤のまとめと考察（2026-09-26 区切り）

**経緯**
1. 旧来の Codex 経路（候補の絞り込み＋費用ゲート）は、標準 Codex では候補が3件で Jev が一度も呼ばれない。Jev を強制すると、ツール一覧が変わってキャッシュが崩れた。
2. jev-gateway の Codex での効果の報告を受け、jev-gateway 本体を公表条件（Codex 0.154、`gpt-5.6-sol`、素の環境）で5ペア再現した。上流の要求 −31%・入力 −30%・出力 −31%（中央値）で、公表の傾向を再現できた。
3. 主指標から Jev を除外した（Jev は安価なため、利用者の指示）。この変更で、Jev の費用を理由にした過去の赤字判定は、上流だけで再評価が必要になった。
4. jev-gateway の `forced`／`none` 方式を `JEV_CODEX_STEER`（既定 off）として移植した。ツール一覧を保ったまま `tool_choice` だけを書き換える。
5. `gpt-5.6-terra`／`medium`、Codex 0.158 の事前登録6ペアは、削減率中央値 −37.08%（改善1・悪化5）で不採用。要求数が最大で約2倍に増えた。途中、Codex 内部の `spawn_agent` 起動失敗をサブエージェント呼出しと誤検出して全ペアが比較不能になる計測の不具合があり、修正した（`internal/proxy/application.go`）。
6. モデルだけを Sol に替えた診断では、3ペアで中央値 +18.9%（改善2・悪化1）。

**考察**
- 効果はモデルに強く依存するとみられる。Sol では jev-gateway の再現と同じく削減方向、terra では増加方向だった。Codex 0.158 でも Sol は減る方向なので、Codex の版よりモデルの違いが主因と考えられる（Sol は3ペアのみで、未確定）。
- terra の増加は上流要求数の増加による。`exec` の強制で、モデルがテキストで応答を区切れず、ターンが延びた可能性がある（未検証）。
- どちらのモデルでも、ランごとの要求数のばらつきが大きい（Sol の対照で44〜68）。少数のペアでは判定が揺れる。
- 現状の結論：`JEV_CODEX_STEER` は既定 off のまま。モデルごとに有効化する価値があるかは、Sol の事前登録系列（6ペア以上）で確かめる必要がある。

**次にやること（未着手）**
- `gpt-5.6-sol` で、事前登録した6ペアの系列を取る。有効なら、モデル別に既定で有効にする設定を検討する。
- terra で要求数が増える仕組みを、要求ごとの記録から確かめる（強制された `exec` の直前に、モデルがテキスト応答を返していたか）。
- Codex のコンパクション改善は、実セッションでの上限が約0.1〜0.3%のため見送った。計測の修正（合成応答の照合補正、`--codex-native-compaction`）だけ実装済み。

---

## 方針：`gpt-5.6-terra` では jev-routing は何もしない（2026-09-26・利用者の決定、未実装）

利用者の決定により、Codex のモデルが `gpt-5.6-terra` のときは、jev-routing は一切介入せず、要求をそのまま転送する方針とする。理由は2つある。第一に、terra ではツール誘導が事前登録6ペアで上流トークンを増やし（削減率中央値 −37.08%）、改善の見込みがない。第二に、terra は古いモデルで、今後の利用は減る。terra は標準的な性能を持つため、これまでテストの基準モデルに採用していた。

- 対象は、ツール誘導（`JEV_CODEX_STEER`）、ツール結果の切り詰め（`JEV_CODEX_TOOL_OUTPUT_TRUNCATE`、既定 on）、圧縮置換など、Codex 経路のすべての介入である。モデルは要求本文の `model` で判定する想定。
- 注意：ツール結果の切り詰めの採用根拠（`large-facts` の６ペア、中央値 +8.19%）は terra で測ったものである。この方針を実装すると、terra ではその効果も失われる。
- 今後のベンチの基準モデルは terra から別のモデルに移す。候補は、ツール誘導で削減方向が出た `gpt-5.6-sol`。基準モデルを決めてから、事前登録を改める。

## Codex：`gpt-5.6-sol` の事前登録６ペア（2026-09-26）

前回の3ペア診断（+55.3%／−16.2%／+18.9%、中央値+18.9%）は打切りのため採否に使わない。ここから正式に事前登録し、独立に反復する。

**条件**：`gpt-5.6-sol`／`medium`、Codex 0.158、`--tasks chess-bugfix --modes off,on --codex-steer --reps 6`（順序交替）。両側とも同じ `baseline` プロキシ経路、Jev 圧縮置換は無効、ツール結果の切り詰めは既定（on）のまま両側同じ。主指標は上流入力（キャッシュ込み）＋上流出力の削減率 `100×(対照−介入)/対照` の中央値。Jev の使用量は別掲。

**比較可能性の判定（利用者指示により変更）**：使用量欠測・CLIとプロキシの照合失敗・子セッション未確認は、比較不能の理由にしない。両側の品質が満点で、上流の入力・出力の値が両側とも記録されているランだけを比較対象とする（欠けていれば個別に注記し、その1点は中央値の計算から外す）。汚染ランは除外する。

**反復と停止**：6ペアを1系列として実行する。品質悪化があれば理由を記録するが、途中で打ち切らない。6ペア完走後、中央値が採用目安（品質悪化なし・削減率中央値5%以上）を満たすか判定する。満たせば「モデル別に既定で有効化する価値がある」と判断し、満たさなければ「Sol でも既定は off のまま」とする。

**この系列の位置づけ**：診断ではなく正式な事前登録。数値は結果を見た後に変更しない。

---

### `gpt-5.6-sol` の事前登録6ペア：結果と判定（2026-09-26）

保存先は `/tmp/jev-codex-steer-sol-v2/`。12ラン全完了、汚染なし、失敗要求なし。品質は全12ランで満点（36/36）。比較不能の判定（欠測・照合失敗・子セッション未確認）は無視し、上流（入力＋出力、Jev除く）の生のトークン量で判定した（利用者指示）。
...
**削減率の中央値は +19.30%（改善4・悪化2）。事前登録の採用目安（品質悪化なし・中央値5%以上）を満たす。** 介入側の `tool_choice` 内訳（2026-09-26 に proxy-events.json から再集計して訂正）：悪化したペア4は `agent_message`（子エージェント関連の会話）を理由にした転送が48件中45件で forced は4件のみと、誘導が実質的に効かなかった。一方、悪化したペア3は forced 54件・転送4件（いずれも low_confidence）で、誘導が効いたうえで悪化した。改善した4ペアの forced は12〜24件で、そのうちペア2（転送35件中28件）とペア6（同27件中18件）にも `agent_message` による転送が多かった。旧記述は「35件中28件」をペア3の値としていたが、ペア2の値だった。terra の事前登録6ペア（中央値 −37.08%、悪化5/6）とは逆方向であり、効果がモデルに強く依存するという考察を裏付けた。

**判断：`gpt-5.6-sol` では `JEV_CODEX_STEER` をモデル別に既定で有効化する価値があると判定する。** ただし改善4・悪化2で外れ値（ペア1の+70.63%）に中央値が引き上げられている面があり、ばらつきは大きい。実装（モデル名で分岐し、terra は off・sol は on にする）と、その後の実装確認は未着手。

**次にやること（未着手）**
- モデル名で既定値を分岐する実装（`gpt-5.6-terra` は off 固定、`gpt-5.6-sol` は on 固定、他モデルは要判断）。
- 実装後、同条件で確認系列（6ペア以上）を取り、実装が意図どおり動くことを確かめる。
- 他の Codex モデル（luna、astra 等）でも同様の診断・事前登録を行うか判断する。

---

## Codex：`gpt-6-luna` の事前登録６ペア（2026-09-26）

terra（中央値−37.08%、不採用）・sol（中央値+19.30%、採用目安を満たす）に続き、`gpt-6-luna` で同じ検証を行う。

**条件**：`gpt-6-luna`／`medium`、Codex 0.158、`--tasks chess-bugfix --modes off,on --codex-steer --reps 6`（順序交替）。両側とも同じ `baseline` プロキシ経路、Jev 圧縮置換は無効、ツール結果の切り詰めは既定（on）のまま両側同じ。主指標は上流入力（キャッシュ込み）＋上流出力の削減率 `100×(対照−介入)/対照` の中央値。Jev の使用量は別掲。

**比較可能性の判定**：使用量欠測・CLIとプロキシの照合失敗・子セッション未確認は比較不能の理由にしない。両側の品質が満点で、上流の入力・出力の値が両側とも記録されているランだけを比較対象とする。汚染ランは除外する。

**反復と停止**：6ペアを1系列として実行し、途中で打ち切らない。6ペア完走後、中央値が採用目安（品質悪化なし・削減率中央値5%以上）を満たすか判定する。

---

## Claude Code：`claude-opus-5-5` の advise 再検証（2026-09-26）

過去の `claude-sonnet-5`／`dual-facts` の判定（総トークン中央値−27.25%、当時はJev込み指標）を、主指標からJevを除外した現在の定義で、モデルを `claude-opus-5-5`、課題を `chess-bugfix`（Codex 側の検証と揃える）に替えて再検証する。

**条件**：`claude-opus-5-5`／`medium`、`--tasks chess-bugfix --modes off,on --reps 6`（順序交替）。`--claude-clear` は指定しない（advise 単体を測る。ベンチは `agent=claude` かつ `mode=on` で自動的に `JEV_CLAUDE_ADVISE=on` を設定する）。主指標は上流入力（キャッシュ込み）＋上流出力の削減率 `100×(対照−介入)/対照` の中央値。Jev の使用量は別掲。

**比較可能性の判定**：既存のベンチの比較不能判定（欠測・照合失敗・子セッション未確認）をそのまま適用する。比較可能ペアが6未満なら、事前登録どおり同条件でペアを追加し、比較可能6で止める（総ペア上限10）。上限でも届かなければ判定不能とする。

**採否**：品質悪化なし・比較可能6ペア・削減率中央値5%以上を採用目安とする。

### `gpt-6-luna` の事前登録6ペア：結果と判定（2026-09-26）

保存先は `/tmp/jev-codex-steer-luna/`。12ラン全完了。上流（入力＋出力、Jev除く）の生のトークン量で判定した。
...
**削減率の中央値は −50.78%（改善1・悪化5）。不採用。** terra と同じく、上流の要求数が増えてトークンが増えた。

### Claude Code：`claude-opus-5-5` の advise 再検証：結果と判定（2026-09-26）

保存先は `/tmp/jev-claude-advise-opus/`。12ラン全完了、6ペアすべて比較可能（欠測・照合失敗・子セッション未確認・汚染は0件）。品質は全ランで満点（36/36）。
...
**削減率の中央値は +1.67%（改善3・悪化3）。採用目安（5%以上）に届かず不採用。** Jev の使用量（別掲）は介入側の入力 30,210〜69,290。

## 最終結論とプロジェクトの終了（2026-09-26）

これ以上の深掘りでは効果が見込めないと判断し、プロジェクトを終了してリポジトリをアーカイブする。

**Jev によるツール選択・誘導**：品質を保ったまま上流トークンを減らせたのは Codex の `gpt-5.6-sol`（事前登録6ペア、中央値 +19.30%）だけだった。同じ方式で `gpt-5.6-terra` は −37.08%、`gpt-6-luna` は −50.78% と逆に増えた。Claude Code の助言は `claude-opus-5-5` で +1.67%（ほぼ0）、`claude-sonnet-5` でも上流の差はほぼ0だった。効果はモデルに強く依存し、汎用的な削減手段にはならない。

**効果が確認できたもの（文脈を小さくする方式）**
- Claude Code：ネイティブ context editing（`clear_tool_uses_20250919`）＋ Jev の消去ゲート。消去が起きたランで同一経路の純削減の中央値 11.1%。実セッション30日分の上限推定は全体の約6.3%。既定は無効。
- Codex：大きなツール結果の決定的な切り詰め。負荷試験 `large-facts` で中央値 +8.19%（区間はゼロをまたぐ）。既定で有効。

**効果がなかったもの**：Claude Code の Jev による `Read` 置換（上流のみでも閾値0.7で約+1.4%の上限）、Jev を使わない決定的なツール呼出の合成（節約上限0.28%）、Codex の圧縮置換と圧縮閾値の変更（実セッションの上限0.1〜0.3%、閾値比較 −0.75%）。

**計測上の教訓**：同一条件同士でも実行間の総トークン差が大きく散る（Claude で −271%〜+90%）。結果を見る前の事前登録、ペア比較、同一経路の指標が不可欠だった。Jev の使用量は安価なため主指標から除外した。
