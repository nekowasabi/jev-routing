...
**削減率の中央値は +19.30%（改善4・悪化2）。事前登録の採用目安（品質悪化なし・中央値5%以上）を満たす。** 介入側の `tool_choice` 内訳は、悪化した2ペア（3・4）で `agent_message`（子エージェント関連の会話）を理由にした転送が多く（それぞれ35件中28件、48件中45件）、誘導が実質的に効かなかった回だった。改善した4ペアは forced が主体（12〜24件）。terra の事前登録6ペア（中央値 −37.08%、悪化5/6）とは逆方向であり、効果がモデルに強く依存するという考察を裏付けた。
