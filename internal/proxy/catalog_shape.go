...
func catalogShape(body []byte) *CatalogShape {
	...
	for _, key := range []string{"messages", "input"} {
		for _, item := range asSlice(root[key]) {
			historyTypes[key+"."+historyShapeType(item)]++
	...
	return &CatalogShape{Keys: shapeKeys(root), ToolsType: shapeType(root["tools"]), Location: location,
