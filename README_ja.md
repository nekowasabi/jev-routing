...
> **このリポジトリはアーカイブ済みです（2026-09-26）。** 検証の結果、Jev によるツール選択・誘導では、AI エージェントのトークン使用量を汎用的に減らせないと判断し、開発を終了しました。結論と試行錯誤の記録を共有するために残しています。詳しい経緯と全データは [RESEARCH.md](RESEARCH.md)（英語）を参照してください。

## 結論

**目的**：品質を保ったまま、Claude Code や Codex などの AI エージェントが使う上流トークン（親・子の入力＋出力）を減らすこと。Jev（TypeSafe）の使用量は安価なため指標に含めていない。

**Jev によるツール選択・誘導は、汎用的な削減手段にならなかった。** 効果はモデルに強く依存し、同じ方式で逆方向に振れる。

| 対象 | 方式 | 事前登録6ペアの削減率（中央値） | 判定 |
|---|---|---:|---|
| Codex `gpt-5.6-sol` | ツール一覧を保ったまま `tool_choice` を強制（jev-gateway 方式の移植） | **+19.30%** | 効果あり |
| Codex `gpt-5.6-terra` | 同上 | −37.08%（増加） | 不採用 |
| Codex `gpt-6-luna` | 同上 | −50.78%（増加） | 不採用 |
| Claude Code `claude-opus-5-5` | 次のツールの助言（`advise`） | +1.67% | 不採用 |
| Claude Code `claude-sonnet-5` | 同上 | 上流の差はほぼ0 | 不採用 |

増加したモデルでは、ツールの強制によって上流への要求数が最大で約2倍に増えた。

**効果が確認できたのは、文脈を小さくする方式だった。**

| 対象 | 方式 | 効果 | 既定 |
|---|---|---|---|
| Claude Code | Anthropic ネイティブの context editing（`clear_tool_uses_20250919`）＋ Jev の消去ゲート | 消去が起きたランで同一経路の純削減 中央値 11.1%。実セッションでの上限推定は全体の約6.3% | 無効（`JEV_CLAUDE_CLEAR_TOOL_USES=on` で有効） |
| Codex | 大きなツール結果の決定的な切り詰め（20,000バイト超の中央を省略） | 負荷試験で中央値 +8.19%（区間はゼロをまたぐ） | 有効（`JEV_CODEX_TOOL_OUTPUT_TRUNCATE=off` で無効） |

---
**トークン量の実測（上流の入力＋出力、Jev を除く。各系列は事前登録した6ペア、課題は `chess-bugfix`。ツール結果の切り詰めのみ `large-facts`）**

| 系列 | 対照の合計 | 介入の合計 | 合計の差 | 1ランの中央値 対照→介入 | ペアごとの削減率の中央値 | 改善/悪化 |
|---|---:|---:|---:|---|---:|---|
| Codex `gpt-5.6-sol`・ツール誘導 | 15,127,871 | 11,509,869 | **−23.92%** | 2,339,036 → 1,912,187 | +19.30% | 4/2 |
| Codex `gpt-5.6-terra`・ツール誘導 | 6,194,181 | 8,266,032 | +33.45%（増加） | 968,075 → 1,286,876 | −37.08% | 1/5 |
| Codex `gpt-6-luna`・ツール誘導 | 3,371,403 | 5,510,014 | +63.43%（増加） | 497,227 → 812,886 | −50.78% | 1/5 |
| Claude Code `claude-opus-5-5`・助言 | 701,885 | 680,083 | −3.11% | 118,671 → 119,439 | +1.67% | 3/3 |
| Codex `gpt-5.6-terra`・ツール結果の切り詰め | 4,179,472 | 3,959,848 | **−5.25%** | 666,336 → 646,678 | +8.19% | 4/2 |

「合計の差」は6ペアの対照・介入それぞれの合計を比べたもの（マイナスが削減）。「削減率」はペアごとの `100×(対照−介入)/対照` の中央値（プラスが削減）。Claude Code の context editing は、消去の有無で経路が大きく変わるため、ラン間の合計ではなく同一経路の純削減（中央値 11.1%）で評価した。
---

**効果がなかったもの**：Jev による `Read` の置換、Jev を使わない決定的なツール呼び出しの合成、Codex の圧縮の置換と圧縮閾値の変更（実セッションで圧縮はほとんど起きず、上限は0.1〜0.3%）。

**計測上の教訓**：同じ条件で走らせても、実行間の総トークン差は大きく散る（Claude Code で −271%〜+90%）。単発の比較や非ペアの中央値では判断できない。結果を見る前に判定条件を事前登録し、ペアで比較することが不可欠だった。

---
（ここから下は、アーカイブ前の README の内容です。）
---
...
Anthropic API は、手動の extended thinking 中と、Claude Opus 5.5 ではどの thinking モードでも `tool_choice` の強制を拒否します（Sonnet 5 など他モデルの adaptive thinking では強制できます）。ツール一覧を変えるとプロンプトキャッシュ全体が、`tool_choice` を変えるとメッセージ部分（会話履歴）のキャッシュが無効になります。残るのは次のツールを誘導ではなく助言することだけで、助言は上流の要求や履歴の大きさを減らせません。増えるのはターンごとに積み上がる Jev の判定コストだけです。
...
Codex の要求に含まれるツール結果（`function_call_output` など）のうち 20,000 バイトを超えるものを、先頭 10,000 バイト・末尾 10,000 バイトと省略注記に置き換えます。注記は、省略部分が必要なら `sed -n` や `rg` で絞って再取得するようモデルに促します。Codex は毎要求で履歴全体を再送するため、最新の結果だけでなく履歴の全件に毎回同じ規則を適用し、プロンプトキャッシュの接頭辞を保ちます。（2026年9月にこのプロキシ経由で観測した挙動です。現在の openai/codex は WebSocket 経由で `previous_response_id` を使い差分だけを送る場合があります。）Jev は呼びません。
