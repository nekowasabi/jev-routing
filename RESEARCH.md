...
2. [Approaches examined](#2-approaches-examined)
3. [Trial and error and data](#3-trial-and-error-and-data)
...
- Claude Code runs with extended thinking enabled, and the Anthropic API rejects tool forcing (`tool_choice`) during thinking. Furthermore, changing the tool list or `tool_choice` invalidates the prompt cache. This means all the proxy can do is advise the next move without removing tools.
...
**The median reduction rate is +19.30% (4 improved, 2 worsened). Meets the pre-registered adoption criterion (no quality degradation, median ≥5%).** In the `tool_choice` breakdown on the intervention side, the two worsened pairs (3, 4) had many forwards due to `agent_message` (subagent-related conversation) reasons (28 of 35 and 45 of 48, respectively), where steering was effectively not applied. The 4 improved pairs were dominated by forced (12-24 instances). This is the opposite direction from terra's pre-registered 6 pairs (median −37.08%, 5/6 worsened), supporting the view that the effect depends strongly on the model.
